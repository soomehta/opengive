 ...
 */
import { eq, sql, and, or, lt, desc } from 'drizzle-orm';
...
function buildCursorCondition(cursor) {
    return or(lt(organizations.createdAt, new Date(cursor.createdAt)), and(eq(organizations.createdAt, new Date(cursor.createdAt)), lt(organizations.id, cursor.id)));
}
/**
 ...
 */
export async function searchOrganizations(db, params) {
    const limit = Math.min(params.limit ?? 25, 100);
    const conditions = [];
    // Full-text filter
    if (params.query && params.query.trim().length > 0) {
        conditions.push(sql `${organizations.searchVector} @@ plainto_tsquery('english', ${params.query})`);
    }
    // Equality filters
    if (params.country) {
    ...
    if (params.status) {
        conditions.push(eq(organizations.status, params.status));
    }
    ...
    const rows = await (params.query && params.query.trim().length > 0
        ? db
            ...
            .from(organizations)
            .where(whereClause)
            .orderBy(desc(sql `ts_rank(${organizations.searchVector}, plainto_tsquery('english', ${params.query}))`), desc(organizations.createdAt), desc(organizations.id))
            .limit(limit + 1)
        : db
            .select()
            .from(organizations)
 ...
 */
export async function listOrganizations(db, params) {
    ...
    if (params.orgType) {
        conditions.push(eq(organizations.orgType, params.orgType));
    }
    if (params.status) {
        conditions.push(eq(organizations.status, params.status));
    }
