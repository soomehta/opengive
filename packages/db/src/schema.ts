 ...
 */
const geography = customType<{ data: unknown; driverData: string }>({
  dataType() {
...
export const organizations = pgTable(
  ...
  (table) => [
    ...
    uniqueIndex('organizations_registry_source_registry_id_key').on(
      table.registrySource,
      table.registryId,
    ),
  ],
);
...
export const financialFilings = pgTable(
  ...
  (table) => [
    ...
    uniqueIndex(
      'financial_filings_organization_id_fiscal_year_filing_type_key',
    ).on(table.organizationId, table.fiscalYear, table.filingType),
  ],
);
...
export const organizationScores = pgTable(
  'organization_scores',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    organizationId: uuid('organization_id').references(() => organizations.id, {
  ...
  (table) => [
    ...
    uniqueIndex(
      'organization_scores_organization_id_fiscal_year_methodology_version_key',
    ).on(table.organizationId, table.fiscalYear, table.methodologyVersion),
  ],
);
...
export const entityMatches = pgTable(
  ...
  (table) => [
    ...
    uniqueIndex('entity_matches_org_a_id_org_b_id_key').on(
      table.orgAId,
      table.orgBId,
    ),
  ],
...
export const embeddings = pgTable(
  ...
  (table) => [
    ...
    uniqueIndex(
      'embeddings_entity_type_entity_id_model_name_model_version_key',
    ).on(
      table.entityType,
      table.entityId,
      table.modelName,
...
export const userProfiles = pgTable('user_profiles', {
  ...
  role: text('role', { enum: ['viewer', 'analyst', 'admin'] }).default(
    'viewer',
  ),
